...
static SERVER_END_OF_HANDSHAKE_PAYLOAD: &str =
    "1403030001011603030020b5abee4e71b1dfd471e32c279d8f25d82279c5082707d6e33992ec94893af2bd";
        ...
        Protocol::Tls(vec![
            ...
            tls::Record {
                ...
                data: tls::Data::HandshakeProtocol(tls::HandshakeProtocol::Encrypted(&[
                    142, 96, 65, 8, 59, 137, 255, 73, 208, 104, 236, 47, 115, 93, 157, 216, 218,
                    14, 40, 107, 95, 155, 132, 177, 53, 190, 14, 76, 245, 56, 196, 9
                ])),
            }
        ]),
        protocol
    ...
    assert_eq!(
        Protocol::Tls(vec![
            ...
            tls::Record {
                ...
                version: "1.2".to_string(),
                data: tls::Data::HandshakeProtocol(tls::HandshakeProtocol::Encrypted(&[
                    181, 171, 238, 78, 113, 177, 223, 212, 113, 227, 44, 39, 157, 143, 37, 216, 34,
                    121, 197, 8, 39, 7, 214, 227, 57, 146, 236, 148, 137, 58, 242, 189
                ])),
            }
        ]),
        protocol
    ...
    assert_eq!(
        Protocol::Tls(vec![tls::Record {
            ...
            data: tls::Data::Encrypted(&[
                87, 100, 221, 126, 25, 88, 50, 86, 62, 43, 162, 6, 231, 115, 182, 131, 72, 30, 108,
                86, 28, 61, 180, 236, 51, 232, 241, 215, 222, 237, 245, 213, 91, 89, 34, 131, 160,
                38, 255, 121, 57, 207, 93, 104, 72, 32, 123, 214, 82, 241, 203
            ]),
        }]),
        protocol
    );
...
fn tls_parser_encrypted_alert() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        Protocol::Tls(vec![tls::Record {
            ...
            version: "1.2".to_string(),
            data: tls::Data::Encrypted(&[
                154, 108, 202, 241, 127, 155, 144, 176, 114, 132, 23, 4, 53, 195, 229, 101, 205,
                255
            ]),
        }]),
        protocol

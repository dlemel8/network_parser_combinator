...
fn extension_parser<'a>(client_hello: bool) -> impl Parser<'a, Extension> {
    one_of(vec![
        ...
        one_of(vec![
            byte_parser(0)
                .and(byte_parser(0))
                .map(|_| Extension::ServerName),
            byte_parser(0)
                .and(byte_parser(1))
                .map(|_| Extension::MaxFragmentLength),
            byte_parser(0)
                .and(byte_parser(2))
                .map(|_| Extension::ClientCertificate),
            byte_parser(0)
                .and(byte_parser(3))
                .map(|_| Extension::TrustedCaKeys),
            byte_parser(0)
                .and(byte_parser(4))
                .map(|_| Extension::TruncatedHMac),
            byte_parser(0)
                .and(byte_parser(5))
                .map(|_| Extension::StatusRequest),
            byte_parser(0)
                .and(byte_parser(6))
                .map(|_| Extension::UserMapping),
            byte_parser(0)
                .and(byte_parser(7))
                .map(|_| Extension::ClientAuthz),
            byte_parser(0)
                .and(byte_parser(8))
                .map(|_| Extension::ServerAuthz),
            byte_parser(0)
                .and(byte_parser(9))
                .map(|_| Extension::CertType),
            byte_parser(0)
                .and(byte_parser(0xa))
                .map(|_| Extension::SupportedGroups),
            byte_parser(0)
                .and(byte_parser(0xb))
                .map(|_| Extension::EcPointFormats),
            byte_parser(0).and(byte_parser(0xc)).map(|_| Extension::Srp),
            byte_parser(0)
                .and(byte_parser(0xd))
                .map(|_| Extension::SignatureAlgorithms),
            byte_parser(0)
                .and(byte_parser(0xe))
                .map(|_| Extension::UseSrtp),
            byte_parser(0)
                .and(byte_parser(0xf))
                .map(|_| Extension::Heartbeat),
            byte_parser(0)
                .and(byte_parser(0x10))
                .map(|_| Extension::ApplicationLayerProtocolNegotiation),
            byte_parser(0)
                .and(byte_parser(0x11))
                .map(|_| Extension::StatusRequestV2),
            byte_parser(0)
                .and(byte_parser(0x12))
                .map(|_| Extension::SignedCertificateTimestamp),
            byte_parser(0)
                .and(byte_parser(0x13))
                .map(|_| Extension::ClientCertificateType),
            byte_parser(0)
                .and(byte_parser(0x14))
                .map(|_| Extension::ServerCertificateType),
            byte_parser(0)
                .and(byte_parser(0x15))
                .map(|_| Extension::Padding),
            byte_parser(0)
                .and(byte_parser(0x16))
                .map(|_| Extension::EncryptThenMac),
            byte_parser(0)
                .and(byte_parser(0x17))
                .map(|_| Extension::ExtendedMasterSecret),
            byte_parser(0)
                .and(byte_parser(0x18))
                .map(|_| Extension::TokenBinding),
            byte_parser(0)
                .and(byte_parser(0x19))
                .map(|_| Extension::CachedInfo),
            byte_parser(0)
                .and(byte_parser(0x1c))
                .map(|_| Extension::RecordSizeLimit),
            byte_parser(0)
                .and(byte_parser(0x23))
                .map(|_| Extension::SessionTicketTLS),
            byte_parser(0)
                .and(byte_parser(0x2d))
                .map(|_| Extension::PskExchangeModes),
            byte_parser(0)
                .and(byte_parser(0x33))
                .map(|_| Extension::KeyShare),
            byte_parser(0xff)
                .and(byte_parser(1))
                .map(|_| Extension::RenegotiationInfo),
        ])
...
fn client_hello_parser<'a, P1, P2>(
    version_parser: impl Fn() -> P1,
    client_context_parser: impl Fn() -> P2,
) -> impl Parser<'a, HandshakeProtocol<'a>>
where
    P1: Parser<'a, Version> + 'a,
    P2: Parser<'a, ()> + 'a,
{
    version_parser()
        .skip(32) // random
        .and(client_context_parser())
...
fn server_hello_parser<'a, P>(
    version_parser: impl Fn() -> P,
) -> impl Parser<'a, HandshakeProtocol<'a>>
where
    P: Parser<'a, Version> + 'a,
{
    version_parser()
        .skip(32) // random
...
pub(crate) fn handshake_parser<'a, P1, F1, P2, F2, P3, F3>(
    size_header_parser: F1,
    version_parser: F2,
    client_context_parser: F3,
) -> impl Parser<'a, HandshakeProtocol<'a>>
where
    P1: Parser<'a, usize> + 'a,
    F1: Fn(usize, bool) -> P1,
    ...
    F3: Fn() -> P3 + Copy + 'a,
{
    one_of(vec![
        byte_parser(1)
            ...
            .then(move |(_, size)| {
                client_hello_parser(version_parser, client_context_parser).skip_to(size)
            }),
        byte_parser(2)
            .and(size_header_parser(3, false))
...
pub(crate) fn data_parser<'a, P>(
    content_type: ContentType,
    handshake_parser: impl Fn() -> P,
) -> impl Parser<'a, Data<'a>>
where
    P: Parser<'a, HandshakeProtocol<'a>> + 'a,
{
    move |input: &'a [u8]| match content_type {
...
mod tests {
    ...
    use crate::tls;
    use crate::tls::{client_context_parser, tls_handshake_parser};
    ...
    #[test]
    fn content_type_parser_on_input_with_known_value_return_content_type(
    ...
    #[test]
    fn extension_parser_on_client_supported_versions_input_return_extension(
    ) -> Result<(), Box<dyn Error>> {
    ...
    #[test]
    fn extension_parser_on_server_supported_versions_input_return_extension(
    ) -> Result<(), Box<dyn Error>> {
    ...
    fn client_hello_parser_on_not_enough_input_return_err() -> Result<(), Box<dyn Error>> {
        ...
        input[39] = 3;
        let result =
            tls::client_hello_parser(tls::version_parser, client_context_parser).parse(&input);
        assert!(result.is_err());
        Ok(())
    }

    #[test]
    fn client_hello_parser_on_valid_no_extension_input_return_handshake(
    ) -> Result<(), Box<dyn Error>> {
        let mut input = [0; 40];
        input[0] = 3;
        input[1] = 3;
        ...
        let result =
            tls::client_hello_parser(tls::version_parser, client_context_parser).parse(&input)?;
        assert_eq!(
            tls::HandshakeProtocol::ClientHello("1.2".to_string(), 0, 0, empty),
            result.parsed
        );
    ...
    #[test]
    fn client_hello_parser_on_valid_with_extension_input_return_handshake(
    ) -> Result<(), Box<dyn Error>> {
        let mut input = [0; 50];
        input[0] = 3;
        input[1] = 3;
        ...
        let result =
            tls::client_hello_parser(tls::version_parser, client_context_parser).parse(&input)?;
        assert_eq!(
            tls::HandshakeProtocol::ClientHello(
                "1.2".to_string(),
                1,
                2,
    ...
    #[test]
    fn server_hello_parser_on_valid_no_extension_input_return_handshake(
    ) -> Result<(), Box<dyn Error>> {
        let mut input = [0; 45];
        input[0] = 3;
        input[1] = 3;
        ...
        let result = tls::server_hello_parser(tls::version_parser).parse(&input)?;
        assert_eq!(
            tls::HandshakeProtocol::ServerHello("1.2".to_string(), empty),
            result.parsed
        );
        assert_eq!([0; 2], result.remaining);
        Ok(())
    ...
    #[test]
    fn server_hello_parser_on_valid_with_extension_input_return_handshake(
    ) -> Result<(), Box<dyn Error>> {
        let mut input = [0; 46];
        input[0] = 3;
        input[1] = 3;
        ...
        let result = tls::server_hello_parser(tls::version_parser).parse(&input)?;
        assert_eq!(
            tls::HandshakeProtocol::ServerHello(
                "1.2".to_string(),
    ...
    #[test]
    fn handshake_parser_on_input_with_known_type_return_specific_handshake(
    ) -> Result<(), Box<dyn Error>> {
    ...
    #[test]
    fn data_parser_on_encrypted_content_type_return_data_with_all_input(
    ) -> Result<(), Box<dyn Error>> {
        let input: [u8; 3] = [3, 3, 7];
        let result =
            tls::data_parser(tls::ContentType::Heartbeat, tls_handshake_parser).parse(&input)?;
    ...
    #[test]
    fn data_parser_on_change_cipher_spec_content_type_and_empty_input_return_err(
    ) -> Result<(), Box<dyn Error>> {
        let result =
            tls::data_parser(tls::ContentType::ChangeCipherSpec, tls_handshake_parser).parse(b"");
        assert!(result.is_err());
        Ok(())
    }

    #[test]
    fn data_parser_on_change_cipher_spec_content_type_and_invalid_input_return_err(
    ) -> Result<(), Box<dyn Error>> {
        let result =
            tls::data_parser(tls::ContentType::ChangeCipherSpec, tls_handshake_parser).parse(b"a");
        assert!(result.is_err());
        Ok(())
    }

    #[test]
    fn data_parser_on_change_cipher_spec_content_type_and_valid_input_return_data(
    ) -> Result<(), Box<dyn Error>> {
        let input: [u8; 3] = [1, 2, 3];
        let result = tls::data_parser(tls::ContentType::ChangeCipherSpec, tls_handshake_parser)
            .parse(&input)?;
    ...
    #[test]
    fn data_parser_on_handshake_content_type_and_unknown_input_return_encrypted(
    ) -> Result<(), Box<dyn Error>> {
        let input: [u8; 4] = [88, 1, 3, 4];
        let result =
            tls::data_parser(tls::ContentType::Handshake, tls_handshake_parser).parse(&input)?;
        assert_eq!(
            tls::Data::HandshakeProtocol(tls::HandshakeProtocol::Encrypted(&[88, 1, 3, 4])),
            result.parsed
        );
        assert!(result.remaining.is_empty());
        Ok(())
    ...
    #[test]
    fn data_parser_on_handshake_content_type_and_valid_input_return_data(
    ) -> Result<(), Box<dyn Error>> {
        let input: [u8; 7] = [16, 0, 0, 1, 1, 2, 3];
        let result =
            tls::data_parser(tls::ContentType::Handshake, tls_handshake_parser).parse(&input)?;
        assert_eq!(
            tls::Data::HandshakeProtocol(tls::HandshakeProtocol::ClientKeyExchange),
            result.parsed
        );
        assert_eq!([2, 3], result.remaining);
        Ok(())

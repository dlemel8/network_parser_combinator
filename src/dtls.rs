...
fn handshake_size_header_parser<'a>(
    header_size_in_bytes: usize,
    consume: bool,
) -> impl Parser<'a, usize> {
    size_header_parser(header_size_in_bytes, consume).skip(8) // message sequence + fragment offset + fragment size
}
...
fn dtls_handshake_parser<'a>() -> impl Parser<'a, tls::HandshakeProtocol<'a>> {
    move |input: &'a [u8]| {
        tls::handshake_parser(
            handshake_size_header_parser,
            version_parser,
            client_context_parser,
        )
        .parse(&input)
        .or_else(|_| hello_verify_request_parser().parse(&input))
    }
}
